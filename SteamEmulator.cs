    ...
    static class SteamEmulator
    {
        ...
        private static bool SetupGameEmu(GameConfig app)
        {
            ...
            try
            {
                ...
                {
                    if (File.Exists(Path.Combine(game_emu_folder, "steam_settings", "disable_overlay.txt")))
                        File.Delete(Path.Combine(game_emu_folder, "steam_settings", "disable_overlay.txt"));
                }
                if (app.DisableNetworking)
                {
                ...
                {
                    if (File.Exists(Path.Combine(game_emu_folder, "steam_settings", "disable_networking.txt")))
                        File.Delete(Path.Combine(game_emu_folder, "steam_settings", "disable_networking.txt"));
                }
                if (app.Offline)
                {
                ...
                {
                    if (File.Exists(Path.Combine(game_emu_folder, "steam_settings", "offline.txt")))
                        File.Delete(Path.Combine(game_emu_folder, "steam_settings", "offline.txt"));
                }
            ...
            try
            {
                using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    /*foreach (CAchievement ach in schema.game.availableGameStats.achievements)
                    {
                        ...
                        ach.icon = Path.Combine("images", ach.name + ".jpg");
                        // download icon gray
                        using (WebClient wc = new WebClient())
                        {
